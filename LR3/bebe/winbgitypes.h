...
#ifndef STDAFX_H_
#include "winbgi.h"         // Provides default structures
#endif
...
struct WindowData
{
	...
	string title;               // Title at the top of the window
	queue<TCHAR> kbd_queue;     // Queue of keyboard characters
	...
	queue<POINTS> clicks[WM_MOUSELAST - WM_MOUSEFIRST + 1];     // Array to hold the coordinates of the clicks
...
HDC BGI__GetWinbgiDC(HWND hWnd = nullptr);
void BGI__ReleaseWinbgiDC(HWND hWnd = nullptr);
...
WindowData* BGI__GetWindowDataPtr(HWND hWnd = nullptr);
...
//                            Global Variables
// ---------------------------------------------------------------------------
extern vector<HWND> BGI__WindowTable;  // WindowThread.cpp
